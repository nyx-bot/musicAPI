...
module.exports = {
    ...
    func: async function stream({util, keys, idGen}, req, res) {
        ...
        const seekPlayback = async ({domain, id, json, useFormat, location, stream, alreadyExists, abort, seeked}) => {
            ...
            if(streamClosed) {
                if(abort && typeof abort == `function` && json.nyxData.livestream) abort();
            ...
            let abr = useFormat && useFormat.abr ? useFormat.abr : json && json.abr ? json.abr : 384;
            ...
            const closeFunc = (by) => {
                ...
                if(json.nyxData.livestream) try {
                    abort();
