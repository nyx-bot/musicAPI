...
module.exports = {
    ...
    func: async ({util, keys, idGen}, req, res) => {
        ...
        const seekPlayback = async ({domain, id, json, location, stream, alreadyExists, abort}) => {
            ...
            if(streamClosed) return console.log(`Stream closed before playback could start!`)

            console.log(`seeking`);

            let seekArg = (seek && json && json.nyxData && json.nyxData.livestream ? `00:00` : seek) || `00:00`
            ...
            if(seekArg != `00:00`) args.splice(6, 0, `-ss`, seekArg)
